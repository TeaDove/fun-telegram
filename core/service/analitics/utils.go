...
type nameGetter struct {
	idToUser     map[int64]message_service.UserInChat
	anonymize    bool
...
func (r *Service) getNameGetter(
	usersInChat message_service.UsersInChat,
	anonymize bool,
) nameGetter {
	idToUser := make(map[int64]message_service.UserInChat, len(usersInChat))
